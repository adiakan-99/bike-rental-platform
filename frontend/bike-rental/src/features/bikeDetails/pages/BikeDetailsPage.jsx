...
import { browseBikes, checkAvailability, getBikeDetail } from "../../../api/bikes.js";
...
export function BikeDetailsPage({ bike, criteria, onBack, onDealer, onView, onBook, wished = false, onWish }) {
  ...
  const [availability, setAvailability] = useState({ state: "idle", available: null });
  ...
  useEffect(() => {
    ...
    getBikeDetail(bike.id)
      ...
      .catch((e) => !cancelled && setError(e.userMessage || "Could not load this bike."))
  ...
  useEffect(() => {
    ...
    checkAvailability(bike.id, start, end)
      .then((r) => !cancelled && setAvailability({ state: "done", available: !!r.available }))
      ...
      .catch(() => !cancelled && setAvailability({ state: "error", available: null }));

    return () => { cancelled = true; };
  }, [bike.id, criteria.startDate, criteria.startTime, criteria.endDate, criteria.endTime]);
  ...
  useEffect(() => {
    ...
    browseBikes({ city: criteria.city, category: full.cat, page: 0, size: 8 })
      .then((res) => {
        if (cancelled) return;
        const mapped = (res.content || []).map(cardDtoToBike).filter((b) => b.id !== bike.id);
        registerBikes(mapped);
  ...
  const included = useMemo(() => {
    ...
    const extra = items
      .filter((i) => !INCLUDED.some((it) => i.toLowerCase().includes(it.label.toLowerCase())))
  ...
  const unavailable = availability.state === "done" && availability.available === false;
    ...
    <>
      <div className="bg-white" style={{ borderBottom: "1px solid var(--line)" }}>
        <div className="mx-auto flex max-w-[1200px] items-center gap-1.5 px-4 py-3 text-sm sm:px-6 lg:px-8" style={{ color: "var(--mute)" }}>
          ...
          <button className="br-crumb" onClick={onBack}>{full.cat} Bikes</button><Caret size={14} />
          <span className="font-semibold" style={{ color: "var(--ink)" }}>{full.name}</span>
        </div>
      </div>

      <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6 lg:px-8">
        {error && (
          <div className="br-card mb-4 rounded-2xl px-4 py-3 text-sm" style={{ color: "#b91c1c" }}>
        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          <div className="min-w-0 lg:w-[68%]">
            ...
            <div className="mt-6">
              {full.badge && (
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full px-2.5 py-1 text-[11px] font-bold text-white" style={{ background: BADGE_COLOR[full.badge] || "var(--brand)" }}>
                    {full.badge}
                  </span>
                </div>
              )}
              <h1 className="br-display mt-2.5 text-2xl font-bold sm:text-3xl">{full.name}</h1>
              <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm" style={{ color: "var(--mute)" }}>
                <span className="font-semibold" style={{ color: "#3a4d55" }}>{full.mf}</span>
                <span>·</span><span>{full.cat}</span>
                {full.year && <><span>·</span><span>{full.year}</span></>}
              </div>
              <div className="mt-4 flex items-end gap-3">
                <span className="br-display text-3xl font-bold">
                  {inr(full.price)}<span className="text-base font-medium" style={{ color: "var(--mute)" }}>/day</span>
                </span>
              </div>
              <p className="mt-1 text-sm" style={{ color: "var(--mute)" }}>
                Estimated total for {durationLabel(hours)}:{" "}
                <span className="br-display font-bold" style={{ color: "var(--ink)" }}>{inr(rental)}</span>
              </p>
            </div>
              ...
              {loading ? (
                <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
                  {Array.from({ length: 6 }).map((_, i) => (
                    <div key={i} className="h-16 animate-pulse rounded-xl" style={{ background: "var(--form-bg)" }} />
                  ))}
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
            ...
            {rentalInfo.length > 0 && (
              <section className="mt-8">
                ...
                <div className="br-card grid grid-cols-1 gap-x-6 gap-y-3 rounded-2xl p-5 shadow-sm sm:grid-cols-2">
                  {rentalInfo.map((r) => (
                    <div key={r.label} className="flex items-center justify-between gap-3" style={{ borderBottom: "1px dashed var(--line)", paddingBottom: 8 }}>
                      <span className="flex items-center gap-2 text-sm" style={{ color: "#3a4d55" }}>
                        <r.icon size={15} style={{ color: "var(--brand)" }} />{r.label}
                      </span>
            ...
            {full.services?.description && (
              <section className="mt-8">
                <h2 className="br-display mb-3 text-lg font-bold">About this {full.mf}</h2>
                <p className="whitespace-pre-line text-sm leading-relaxed" style={{ color: "#3a4d55" }}>
                  {full.services.description}
                </p>
              </section>
            )}
            ...
            {included && (
              <section className="mt-8">
                ...
                <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4">
                  {included.map((it) => (
                    <div key={it.label} className="flex items-center gap-2 rounded-xl px-3 py-2.5" style={{ background: it.on ? "#e7f2f9" : "#f5f5f4", opacity: it.on ? 1 : 0.55 }}>
                      <span className="grid h-6 w-6 place-items-center rounded-full text-white" style={{ background: it.on ? "var(--brand)" : "#a3aead" }}>
                        {it.on ? <Check size={13} strokeWidth={3} /> : <X size={13} strokeWidth={3} />}
                      </span>
            ...
            {terms.length > 0 && (
              <section className="mt-8">
                ...
                <div className="flex flex-col gap-2.5">
                  {terms.map((t) => <Accordion key={t.q} {...t} />)}
                </div>
              </section>
            )}
          </div>
              ...
              <div className="br-card rounded-2xl p-5 shadow-sm">
                ...
                {availability.state !== "idle" && (
                  <div
                    ...
                    style={
                      availability.state === "loading" ? { background: "var(--form-bg)", color: "var(--mute)" }
                      : availability.available ? { background: "#e7f2f9", color: "var(--brand-strong)" }
                      : availability.state === "error" ? { background: "#f5f5f4", color: "var(--mute)" }
                      : { background: "#fdf2f2", color: "#b91c1c" }
                    }
                  >
                    {availability.state === "loading" ? "Checking availability…"
                      : availability.available ? <><Check size={13} /> Available for these dates</>
                      : availability.state === "error" ? "Couldn't check availability"
                ...
                <div className="flex flex-col gap-2 text-sm">
                  <div className="flex items-center justify-between"><span style={{ color: "var(--mute)" }}>Rental ({inr(full.price)} × {days})</span><span className="font-semibold">{inr(rental)}</span></div>
                  <div className="flex items-center justify-between"><span style={{ color: "var(--mute)" }}>Security deposit</span><span className="font-semibold" style={{ color: full.deposit === 0 ? "var(--brand)" : "inherit" }}>{full.deposit === 0 ? "Free" : `${inr(full.deposit)} (refundable)`}</span></div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="br-display text-xl font-bold">{inr(total)}</span>
                </div>
                ...
                <div className="mt-2.5 flex gap-2">
                  <button onClick={() => onWish?.(full.id)} className="br-ghost br-display flex flex-1 items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold">
                    <Heart size={16} fill={wished ? "#dc2626" : "none"} color={wished ? "#dc2626" : "currentColor"} /> {wished ? "Saved" : "Wishlist"}
                  </button>
        ...
        {similar.length > 0 && (
          <Carousel title={`More ${full.cat} bikes in ${criteria.city}`}>
            {similar.map((b) => (
              <MiniCard key={b.id} bike={b} city={criteria.city} variant="similar" onView={onView} />
            ))}
          </Carousel>
        )}
      </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 bg-white lg:hidden" style={{ borderTop: "1px solid var(--line)", boxShadow: "0 -6px 20px -12px rgba(8,36,54,.4)" }}>
        <div className="flex items-center justify-between gap-3 px-4 py-3">
          <div className="min-w-0">
            ...
            <p className="text-sm"><span className="br-display font-bold">{inr(full.price)}</span><span className="text-xs" style={{ color: "var(--mute)" }}>/day</span></p>
          </div>
          <button
            onClick={onBook}
            disabled={unavailable}
