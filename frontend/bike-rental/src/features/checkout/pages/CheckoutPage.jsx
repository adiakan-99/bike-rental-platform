...
import { useDealer } from "../../../lib/useDealer.js";
...
export function CheckoutPage({ bike, criteria, onBack, onHome, onConfirmed }) {
  ...
  const dealer = useDealer(bike?.dealer);
  const hours =
    durationHours(
      criteria.startDate,
      criteria.startTime,
  ...
  const [a1, setA1] = useState(false),
    [a2, setA2] = useState(false),
    [a3, setA3] = useState(false);
  ...
  const addonsTotal = ADDONS.filter((a) => addons.has(a.key)).reduce(
    (s, a) => s + a.price,
    0,
  );
  ...
  const applyPromo = () => {
    ...
    if (code === "RIDE100") {
      setCoupon({ code, amount: 100, label: "RIDE100" });
      setPromoMsg({ ok: true, text: "₹100 off applied!" });
    } else if (code === "WEEKEND") {
      setCoupon({
        code,
        amount: Math.round(bike.price * days * 0.1),
        label: "WEEKEND",
      });
      setPromoMsg({ ok: true, text: "10% weekend discount applied!" });
    } else {
      setCoupon(null);
  ...
  const phaseLabel =
    phase === "creating" ? "Creating secure order…" : "Launching Razorpay…";
  ...
  const startPayment = () => {
    ...
    setTimeout(() => {
      ...
      onConfirmed({
        ...
        regNo,
        fare: p,
        addons: ADDONS.filter((a) => addons.has(a.key)),
        coupon,
        paidAt: new Date(),
      });
    }, 2300);
  ...
  const RightSummary = (
    <div className="flex flex-col gap-4">
      ...
      <div className="br-card overflow-hidden rounded-2xl shadow-sm">
        <div className="flex gap-3 p-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs" style={{ color: "#3a4d55" }}>
              {durationLabel(hours)} ·{" "}
              {fmtDateTime(criteria.startDate, "").split("·")[0]}→{" "}
              {fmtDateTime(criteria.endDate, "").split("·")[0]}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="br-card rounded-2xl p-5 shadow-sm">
        ...
        <div className="mt-3 flex flex-col gap-2 text-sm">
          ...
          <Sum
            ...
            value={inr(p.deposit)}
            tip="Fully refundable after the bike is returned and inspected, minus any damages, fines or extra charges."
          />
          <Sum label="Taxes (GST 18%)" value={inr(p.gst)} />
          <Sum
            label="Platform + booking fee"
            value={inr(p.platform + p.booking)}
          />
          <Sum label="Insurance" value={inr(p.insurance)} />
          {p.helmet > 0 && <Sum label="Helmet rental" value={inr(p.helmet)} />}
          {p.addonsTotal > 0 && (
            <Sum label="Add-ons" value={inr(p.addonsTotal)} />
          )}
          <Sum
            label="Discount"
            value={`- ${inr(p.discount)}`}
            color="var(--brand)"
          />
          {p.couponAmt > 0 && (
            <Sum
              label={`Coupon (${coupon.label})`}
              value={`- ${inr(p.couponAmt)}`}
              color="var(--brand)"
            />
          )}
        </div>
        ...
        >
          <span className="br-display text-sm font-bold">
            Amount Payable Now
          </span>
          <span
            className="br-display text-xl font-bold"
      ...
      <div className="br-card rounded-2xl p-5 shadow-sm">
        ...
        >
          <Tag size={13} style={{ color: "var(--brand)" }} /> Promo Code
        </label>
        <div className="flex gap-2">
          <div className="br-field flex flex-1 items-center gap-2 rounded-xl px-3 py-2.5">
            <input
              value={promo}
              onChange={(e) => setPromo(e.target.value)}
              placeholder="Try RIDE100 or WEEKEND"
              className="br-input w-full text-sm"
            />
          </div>
          <button
            onClick={applyPromo}
            className="br-ghost br-display rounded-xl px-4 py-2.5 text-sm font-semibold"
          >
            Apply
          </button>
        </div>
        {promoMsg && (
          <p
            ...
            style={{ color: promoMsg.ok ? "var(--brand)" : "#dc2626" }}
          >
            {promoMsg.ok ? (
      ...
      <div className="br-card rounded-2xl p-5 shadow-sm">
        <div className="flex flex-col gap-2.5">
          ...
          >
            ...
            />{" "}
            I understand the security deposit may be adjusted against damages,
        ...
        <button
          ...
          className="br-btn br-display mt-4 flex w-full items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold"
          style={
            !canPay || busy
              ? {
                  background: busy ? undefined : "#c3d5dd",
                  boxShadow: busy ? undefined : "none",
        ...
        >
          {busy ? (
            <>
              <Loader2 size={16} className="animate-spin" /> {phaseLabel}
            </>
          ) : (
            <>Proceed to Payment · {inr(p.payNow)}</>
          )}
        </button>
        {!canPay && !busy && (
          <p
        ...
        <p
          ...
          style={{ color: "var(--mute)" }}
        >
          <Lock size={12} /> Secured by Razorpay — payment completes in a secure
          modal.
        </p>
        <button
          onClick={onBack}
          disabled={busy}
          className="br-ghost br-display mt-2.5 w-full rounded-xl py-2.5 text-sm font-semibold"
  ...
  return (
    <>
      ...
      >
        <div
          className="mx-auto flex max-w-[1200px] items-center gap-1.5 overflow-x-auto px-4 py-3 text-sm sm:px-6 lg:px-8"
          style={{ color: "var(--mute)" }}
        >
          <button className="br-crumb" onClick={onHome}>
            Home
          </button>
          <Caret size={14} />
          ...
          <Caret size={14} />
          <button className="br-crumb whitespace-nowrap" onClick={onBack}>
            Bike Details
          </button>
          <Caret size={14} />
          <span
      ...
      <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6 lg:px-8">
        <h1 className="br-serif text-3xl font-bold">
          Review & Confirm Booking
        </h1>
        <p className="mt-1 text-sm" style={{ color: "var(--mute)" }}>
          You're almost there — review the details, add extras, and pay
          securely.
        </p>
        ...
        <div className="mt-6 flex flex-col gap-6 lg:flex-row">
          ...
          <div className="flex min-w-0 flex-col gap-5 lg:w-[68%]">
            ...
            >
              <div className="flex flex-col gap-4 sm:flex-row">
                <BikeImage
                  bike={bike}
                  className="h-40 rounded-xl sm:w-56 sm:shrink-0"
                />
                <div className="min-w-0 flex-1">
                  ...
                  <div
                    className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 rounded-xl px-3 py-2.5 text-sm"
                    style={{ background: "var(--form-bg)" }}
                  >
                    ...
                    <span
                      ...
                      style={{ color: "#3a4d55" }}
                    >
                      <Phone size={13} style={{ color: "var(--brand)" }} /> +91{" "}
                      {dealer.rentals ? "98765 43210" : "98765 43210"}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <CheckSection title="Rental Duration" icon={Clock}>
              <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
                ...
                >
                  ...
                  <p className="br-display mt-0.5 text-sm font-bold">
                ...
                <div className="flex flex-col items-center px-2">
                  ...
                  <ArrowRight
                    size={16}
                    className="mt-1 rotate-90 sm:rotate-0"
                    style={{ color: "var(--brand)" }}
                  />
                </div>
                <div
            ...
            <CheckSection title="Pickup Details" icon={MapPin}>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <Row label="Dealer" value={dealer?.name || "Dealer"} />
                  <Row label="City" value={dealer?.city || ""} />
                    <Row label="Contact" value={dealer?.phone || "Contact partner via app"} />
                  <p className="mt-2 text-sm" style={{ color: "#3a4d55" }}>
                    <span className="font-semibold">Address: </span>Shop 14,{" "}
                    {dealer?.area || ""}, {dealer?.city || ""} 411001
                  </p>
                  <div className="mt-3">
                    <p
                    ...
                    >
                      ...
                      <li className="flex items-start gap-2">
                        <Check
                          ...
                          className="mt-0.5 shrink-0"
                        />{" "}
                        Arrive within 30 minutes of scheduled pickup.
                      </li>
                      <li className="flex items-start gap-2">
                        <Check
                          size={15}
                ...
                <div
                  className="relative h-full min-h-[180px] overflow-hidden rounded-xl"
                  style={{
                ...
                >
                  <div
                    ...
                    style={{
                      backgroundImage:
                        "linear-gradient(rgba(15,143,181,.12) 1px,transparent 1px),linear-gradient(90deg,rgba(15,143,181,.12) 1px,transparent 1px)",
                  ...
                  <div className="absolute inset-0 grid place-items-center text-center">
                    <div>
                      ...
                      />
                      <p
                        className="br-display mt-1 text-sm font-bold"
                        style={{ color: "var(--brand-strong)" }}
                      >
                        {dealer.area}, {dealer.city}
                      </p>
                      <p className="text-xs" style={{ color: "#5b7a86" }}>
                        Google Maps preview
                      </p>
                    </div>
                  </div>
            ...
            <CheckSection title="Return Details" icon={RefreshCw}>
              <Row
                ...
                value={`Same as pickup · ${dealer.area}`}
              />
              <Row
                label="Return Date & Time"
                value={fmtDateTime(criteria.endDate, criteria.endTime)}
              />
              <div
                className="mt-2 flex items-start gap-2 rounded-xl px-3 py-2.5 text-sm"
                style={{ background: "#fff7ed", color: "#b45309" }}
              >
                <AlertCircle size={15} className="mt-0.5 shrink-0" />{" "}
                <span>
                  <span className="font-semibold">Late return policy: </span>
                  ₹150 per hour beyond the scheduled return time, deducted from
                  your deposit.
                </span>
              </div>
            </CheckSection>
            ...
            <CheckSection title="Fare Breakdown" icon={Receipt}>
              <div className="flex flex-col gap-2 text-sm">
                <Sum
                  label={`Rental charges (${inr(bike.orig)} × ${days})`}
                  value={inr(p.rentalGross)}
                />
                <Sum
                ...
                <Sum
                  ...
                  value={inr(p.deposit)}
                  tip="Held now and refunded after the ride, minus any deductions for damage, fines or extra usage."
                />
                <Sum
                  label="Platform / convenience fee"
                  value={inr(p.platform)}
                />
                <Sum label="Booking fee" value={inr(p.booking)} />
                <Sum label="Insurance charges" value={inr(p.insurance)} />
                {p.helmet > 0 && (
                  <Sum label="Helmet rental" value={inr(p.helmet)} />
                )}
                <Sum
                  label="Delivery / pickup charges"
                  value={p.delivery === 0 ? "Free" : inr(p.delivery)}
                  color={p.delivery === 0 ? "var(--brand)" : undefined}
                />
                {p.addonsTotal > 0 && (
                  <Sum label="Optional add-ons" value={inr(p.addonsTotal)} />
                )}
                {p.couponAmt > 0 && (
                  <Sum
                    label={`Coupon (${coupon.label})`}
                ...
                {ADDONS.map((a) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center justify-between gap-2 rounded-xl px-3.5 py-3 text-left transition"
                      style={
                        on
                          ? {
                              border: "1.5px solid var(--brand)",
                    ...
                    >
                      <span className="flex items-center gap-2.5 text-sm font-medium">
                        <a.icon size={16} style={{ color: "var(--brand)" }} />{" "}
                        {a.label}
                      </span>
                      <span className="flex items-center gap-2">
                        ...
                        <span
                          ...
                          style={{
                            background: on ? "var(--brand)" : "#cbd5e1",
                          }}
                        >
                          {on ? (
              ...
              <div className="flex flex-col">
                ...
                >
                  <p className="text-sm" style={{ color: "#3a4d55" }}>
                    You pay rental charges, refundable security deposit, and
                    taxes & fees now.
                  </p>
                </TL>
                <TL
                ...
                >
                  <p className="text-sm" style={{ color: "#3a4d55" }}>
                    Possible extra charges: extra kilometres, fuel, traffic
                    challans, damage, missing accessories, late return, or
                    cleaning.
                  </p>
                </TL>
                <TL
                ...
                >
                  <p className="text-sm" style={{ color: "#3a4d55" }}>
                    The bike is inspected and your deposit is refunded after
                    deducting any extra km, challans, damage, late-return or
                    penalty charges.
                  </p>
                  <p
                    ...
                    style={{ color: "var(--brand-strong)" }}
                  >
                    <Clock3 size={14} /> Estimated refund processing: 3–7
                    business days
                  </p>
                </TL>
              </div>
                ...
                {CANCEL_TIERS.map((t) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className="br-display text-sm font-bold"
                        style={{ color: t.color }}
                      >
                        {t.refund}
                      </span>
          ...
          <div className="lg:w-[32%]">
            <div className="lg:sticky lg:top-24">{RightSummary}</div>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        style={{
          ...
          boxShadow: "0 -6px 20px -12px rgba(15,39,51,.4)",
        }}
      >
        <div className="flex items-center justify-between gap-3 px-4 py-3">
          <div>
            ...
            <p
              className="br-display text-lg font-bold"
              style={{ color: "var(--brand)" }}
            >
              {inr(p.payNow)}
            </p>
          ...
          <button
            onClick={startPayment}
            disabled={!canPay || busy}
            className="br-btn br-display flex shrink-0 items-center gap-2 rounded-xl px-6 py-3 text-sm font-semibold"
            style={
              !canPay || busy
                ? {
                    background: busy ? undefined : "#c3d5dd",
          ...
          >
            {busy ? (
              <>
                ...
                {phase === "creating" ? "Processing…" : "Launching…"}
              </>
            ) : (
              "Proceed to Payment"
            )}
