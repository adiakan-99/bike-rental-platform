...
export function ForgotPasswordCard({
  ...
  const [step, setStep] = useState(0); // 0 email · 1 token + new password · 2 done
  const [email, setEmail] = useState(presetEmail);
  const [sentTo, setSentTo] = useState("");
  const [otp, setOtp] = useState(""); // the reset token/code from the email
  ...
  // Step 1: ask the backend to email a reset token. Always advances (server answers the
  ...
  const resend = async () => {
    if (!cooldown) {
      await onForgotPassword?.(sentTo);
      setCooldown(30);
    }
  };

  // Step 2: submit the emailed token + new password to the backend.
  const submitReset = async () => {
    setT({ otp: true, pw: true, cpw: true });
    if (otpErr || pwErr || cpwErr) return;
    setFormErr("");
    setBusy(true);
    const res = await onResetPassword?.(otp.trim(), pw);
    setBusy(false);
    if (res?.ok) setStep(2);
    else
      setFormErr(
        res?.error || "That code is invalid or has expired. Request a new one.",
      );
  };
      ...
      <div className="br-card w-full max-w-md rounded-2xl p-6 shadow-sm sm:p-8">
        ...
        {step === 2 ? (
          <div className="flex flex-col items-center text-center">
            ...
            <h1 className="br-serif br-fade-up br-d1 mt-4 text-2xl font-bold">
              Password updated
            </h1>
            <p
          ...
          <>
            <span
              ...
              style={{ background: "var(--brand)" }}
            >
              {step === 0 ? <Mail size={22} /> : <Lock size={22} />}
            </span>
            <h1 className="br-serif mt-4 text-2xl font-bold">
              {step === 0 ? "Forgot your password?" : "Enter the code we sent"}
            </h1>
            ...
            {step === 0 ? (
              <div className="mt-5 flex flex-col gap-4">
                ...
                <button
                  onClick={verifyEmail}
                  disabled={busy}
                  className="br-btn br-display flex w-full items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold"
                  style={busy ? { opacity: 0.75, cursor: "wait" } : undefined}
                >
                  {busy ? (
                    <>
              ...
              <div className="mt-5 flex flex-col gap-4">
                ...
                >
                  <input
                    ...
                    onBlur={() => setT((p) => ({ ...p, otp: true }))}
                    placeholder="6-digit code"
                    className="br-input w-full text-sm tracking-[0.35em]"
                  />
                </Field>
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <button
                    onClick={resend}
                    disabled={!!cooldown}
                    className="br-display flex items-center gap-1.5 font-semibold"
                    style={{
                      color: cooldown ? "var(--mute)" : "var(--brand-strong)",
