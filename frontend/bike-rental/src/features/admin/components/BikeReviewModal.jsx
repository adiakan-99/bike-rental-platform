...
export function BikeReviewModal({ bike: b, onClose, onDecide, onReject }) {
  ...
  const missing = docList.length
    ? DOC_SET.filter((d) => !docTypes.includes(d))
    : [];
  ...
  return (
    <Modal
      ...
      footer={
        <div className="flex flex-col gap-2.5 sm:flex-row">
          <button
            onClick={() => {
              onDecide(b.id, "approve");
              onClose();
            }}
            className="br-btn br-display rounded-xl py-2.5 text-sm font-semibold sm:flex-1"
          >
            Approve listing
          </button>
          <button
            onClick={() => onReject(b.id)}
            className="br-display rounded-xl py-2.5 text-sm font-semibold sm:flex-1"
            style={{
    ...
    >
      <div
        ...
        style={{ background: CAT_GRADIENT[b.cat] || CAT_GRADIENT.Commuter }}
      >
      ...
      {missing.length > 0 && (
        ...
        >
          ...
          <p className="text-xs" style={{ color: "#7c5410" }}>
            Missing {missing.join(", ")}. Rejecting sends the listing back to
            the partner for resubmission.
          </p>
        </div>
      )}

      <p
        className="br-display mt-4 mb-2 text-xs font-bold uppercase tracking-wide"
      ...
      {docList.length === 0 ? (
        <div
          ...
          style={{ background: "var(--form-bg)", color: "var(--mute)" }}
        >
          <FileText size={15} /> No documents were attached to this submission.
        </div>
      ) : (
        <div className="grid gap-2.5 sm:grid-cols-2">
          {docList.map((d) => (
            <button
              ...
              className="br-doc-card flex items-center gap-3 rounded-xl p-3 text-left transition"
              style={{ border: "1px solid var(--line)", background: "#fff" }}
            >
              <span
                className="grid h-11 w-11 shrink-0 place-items-center rounded-lg"
                style={{
                  background: d.kind === "image" ? "#eef2ff" : "#fef2f2",
                }}
              >
                {d.kind === "image" ? (
              ...
              <span
                className="br-display flex shrink-0 items-center gap-1 text-xs font-semibold"
                style={{ color: "var(--brand-strong)" }}
              >
                <Eye size={13} /> View
              </span>
      ...
      {b.note && (
        <>
          ...
          <p
            ...
            style={{ background: "var(--form-bg)", color: "#3a4d55" }}
          >
            {b.note}
          </p>
        </>
