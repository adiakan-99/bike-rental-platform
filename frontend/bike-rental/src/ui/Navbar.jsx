...
export function Navbar({
  ...
  const Item = ({ icon: Icon, label, sub, onClick, badge }) => (
    <button
      onClick={go(onClick)}
      role="menuitem"
      className="br-option flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition"
    >
      <span
      ...
      <span className="min-w-0 flex-1">
        ...
        {sub && (
          <span className="block text-xs" style={{ color: "var(--mute)" }}>
            {sub}
          </span>
        )}
      </span>
  ...
  const Group = ({ label, children }) => (
    <div className="py-1.5">
      <p
        className="px-3 pb-1 text-[10px] font-bold uppercase tracking-wider"
        ...
        <nav className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center">
            ...
            >
              ...
              <span className="br-display text-lg font-bold">
                Bike<span style={{ color: "var(--brand)" }}>Rental</span>
              </span>
            </button>
          </div>
        </nav>
        ...
        <div className="flex h-16 items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {session && isCustomer && (
              ...
              >
                <History size={15} /> Rental History
              </button>
            )}
            ...
            {!loggedIn && (
              <button
                onClick={onLogin}
                className="br-btn br-display rounded-xl px-4 py-2 text-sm font-semibold sm:px-5"
              >
                Login
              </button>
            )}
            ...
            <div className="relative">
              <button
                ...
                className="flex h-10 items-center rounded-xl px-2 transition"
                style={{
                  border: "1px solid var(--line)",
                  background: open ? "var(--form-bg)" : "#fff",
                }}
              >
                <span
                  className="grid h-6 w-6 place-items-center rounded-full text-white"
                  style={{ background: loggedIn ? "var(--teal)" : "#cbd5e1" }}
                >
                  <User size={13} />
                </span>
              </button>
              ...
              {open && (
                <div
                  ...
                  style={{
                    ...
                    border: "1px solid var(--line)",
                    zIndex: 60,
                    maxHeight: "min(360px, calc(100vh - 6rem))",
                  }}
                >
                  {/* account header */}
                  {loggedIn ? (
                    <button
                      ...
                      className="br-profile-hdr flex w-full shrink-0 items-center gap-3 px-4 py-3 text-left transition"
                      style={{ background: "var(--form-bg)" }}
                    >
                      <span
                  ...
                  <div className="br-scroll flex-1 overflow-y-auto p-1.5">
                    ...
                    {isAdmin && (
                      <Group label="Administration">
                        <Item
                          ...
                          sub="Approve, reject & block dealers"
                          onClick={onAdminPartners}
                        />
                        <Item
                          icon={ShieldCheck}
                          label="Show admins"
                          sub="View the admin team"
                          onClick={() => {
                            setOpen(false);
                            onShowAdmins?.();
                          }}
                        ...
                        <Item
                          icon={UserPlus}
                          label="Add admin"
                          sub="Create a new admin account"
                          onClick={() => {
                            setOpen(false);
                            onAddAdmin?.();
                          }}
                        ...
                        {partnerApproved ? (
                          <Group label="Hosting">
                            <Item
                              ...
                              sub="Bookings, fleet & earnings"
                              onClick={onDealerPortal}
                            />
                            <Item
                              icon={Bike}
                              label="My fleet"
                              sub="Listings and approvals"
                              onClick={onMyFleet || onDealerPortal}
                            />
                            <Item
                              icon={FileText}
                              label="Partner profile"
                              sub="Business, address & payout details"
                              onClick={onPartnerProfile}
                            />
                          </Group>
                        ) : (
                          <Group label="Partner">
                            <Item
                              ...
                              sub="Under review — listings locked"
                              onClick={onDealerPortal}
                            />
                            <Item
                              icon={FileText}
                              label="Partner profile"
                              sub="Review what you submitted"
                              onClick={onPartnerProfile}
                            />
                            <Item
                              icon={UserPlus}
                              label="Complete onboarding"
                              sub="Add or update your details"
                              onClick={onPartnerOnboard}
                            />
                          </Group>
                        )}
                      </>
                    ...
                    {suspended && (
                      <>
                        <div
                          className="mx-1.5 mt-1.5 flex items-start gap-2 rounded-xl px-3 py-2.5"
                        ...
                        >
                          ...
                          <p
                            ...
                            style={{ color: "#7f1d1d" }}
                          >
                            Account suspended — booking, hosting and review
                            tools are locked.
                          </p>
                        </div>
                        <Group label="Account">
                          <Item
                            icon={History}
                            label="My Account"
                            sub="View past bookings (read-only)"
                            onClick={onRentals}
                          />
                        </Group>
                      </>
                      ...
                      <>
                        <Group label="Account">
                          <Item
                            icon={Lock}
                            label="Log in"
                            sub="Access your bookings"
                            onClick={onLogin}
                          />
                          <Item
                            icon={User}
                            label="Create an account"
                            sub="Rent in under 2 minutes"
                            onClick={onRegister}
                          />
                        </Group>
                        ...
                        <Group label="Hosting">
                          <Item
                            icon={Bike}
                            label="List your bikes"
                            sub="Rent out what you own"
                            onClick={onPartner}
                            badge="Earn"
                          />
                          <Item
                    ...
                    {loggedIn && (
                      <>
                        ...
                        <button
                          onClick={go(onLogout)}
                          role="menuitem"
                          className="br-option flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left"
                        >
                          <span
                            className="grid h-8 w-8 shrink-0 place-items-center rounded-lg"
                            style={{ background: "#fee2e2" }}
